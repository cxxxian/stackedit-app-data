...
```
然后去`assimpInstanceLoader.cpp`中的`processMesh`方法就可以改过来了
```cpp
auto geometry = new Geometry(positions, normals, uvs, colors, indices);
auto material = new GrassInstanceMaterial();
```
现在运行和`PhongInstanceMaterial`并没有区别，因为我们只是单纯复制过来的
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg5MzE0ODE5MiwxMTI1NjgyNzQ3LDEzMD
IzMzc0MTksLTE0MDY1MjAxMTAsLTU1MDM5NTkwOSwtNzQ2OTM1
MTg0LC0xNzU3MzYxNTA1LC03OTQzOTk1NDYsLTE4Nzk3NzE1Mj
YsODg4NTE3NjEsLTUxNjE0NDYzNSwtMTkzMjE1NzQyM119
-->
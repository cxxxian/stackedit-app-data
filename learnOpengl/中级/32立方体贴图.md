...
### 3
可以看到`GL_TEXTURE_CUBE_MAP_POSITIVE_X`本质是一个数字，而且右左上下后前按照依次`+1`的方式得到，所以我们在循环中可以直接通过
`GL_TEXTURE_CUBE_MAP_POSITIVE_X + i`得到类型，而不是通过`if`一个一个判断方向，但是前提是

![输入图片说明](/imgs/2025-02-16/lcjjTanbL8VsN5Au.png)
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA1NTc3MzQyNywxNzE0ODY4OTkzLC0xOT

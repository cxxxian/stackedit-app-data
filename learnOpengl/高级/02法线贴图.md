...
```
 ...
 解释一下下面的，`positions`，`uvs`，`normals`，`indices`都没啥好奇怪的，正常声明
 然后下面的tangent计算，是根据公式：
 
![输入图片说明](/imgs/2025-02-20/wUvv2iMGsYQm2KRk.png)

我们只关心`T`，`B`不管
计算完`T`之后用数组存储，然后调用我们先前设计的带有`tangent``geometry`构造函数
```cpp
 void prepare() {
		...
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk5Njg2NTA5LDEzMjM5OTg5NDAsLTEzMj
Q2MTAzMDQsMTA4OTA1NDI2MywtMTQ1MjUxMDY1NCwxMzI1NTY3
MzA4LC0xNzg4MjM4NjUsMTU5NDgwNTE3LC0xMjc2OTg4MTUxLC
01MjA0ODM5NiwxNTYyMTcyOTQyLC02ODA2NjgzODAsMTU0MDg5
MDkzMyw4MDE0MTQ0MjBdfQ==
-->
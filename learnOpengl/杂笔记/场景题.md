...
-   屏幕被划分成固定大小的 tile（比如 16×16 像素）。
    
-   每个 tile 可以在屏幕空间对应一个矩形区域。
    

这样我们要处理的单位就不是「像素」而是「tile」。

----------

#### 2. 光源边界体积（Bounding Volume）

对每个光源（通常是点光源或聚光灯），我们可以定义它的**影响范围**：
...
-   进一步转成 tile 坐标范围（比如从 tileXmin 到 tileXmax，tileYmin 到 tileYmax）。
...
遍历所有光源，找到它影响的 tile 范围，把这个光源 **添加到这些 tile 的光源列表中**。  
通常 GPU 端的实现方式：

1.  **光源 Cull 阶段（Compute Shader）**
    
    -   一个线程组对应一个 tile。
        
    -   在 shader 中，先算出这个 tile 在 view space 下对应的 frustum（一个小视锥）。
        
    -   检查每个光源的包围体是否与这个小视锥相交（Sphere vs Frustum test）。
        
    -   相交 → 把光源的 index 写入 tile 的光源列表。
        
2.  **存储方式**
    
    -   常见是一个结构化缓冲区（Structured Buffer / SSBO），里面存 tile→光源索引的映射。
        
    -   还会有一个偏移表，用来快速找到某个 tile 的光源列表。
        

----------

#### 5. 延迟着色阶段（Lighting Pass）

-   每个像素先确定自己属于哪个 tile（用 gl_FragCoord.xy / tileSize）。
    
-   查 tile 的光源列表。
    
-   只循环这个小列表做光照计算。
    
-   相比传统「对所有光源都遍历」的方式，极大减少了冗余计算。
...
-   如果有上千光源，传统做法每个像素要遍历数百个光源。
    
-   Tiled Deferred 只需要遍历「相关的几十个光源」。
    
-   在现代 GPU 上通常用 **Compute Shader 并行构建 tile 光源表**，效率很高。
...
**全称：Shader Storage Buffer Object**。  
简单说，它就是一种 **可以在 GPU Shader 里随意读写的大块显存缓冲区**。

和常见的 Uniform Buffer Object (UBO) 对比

数据大小
UBO：
一般限制在几十 KB（例如 16KB~64KB）
SSBO：
可以很大，几 MB、几百 MB 都行
...
只读，通常 CPU 更新，GPU 读取
SSBO：
读写，GPU 可以在 shader 内修改

使用场景
UBO：
小规模常量参数（灯光数少、材质参数）
SSBO：
大规模数据表、动态结果存储（光源列表、粒子数据等）

### 在 Tiled Deferred Shading 里的作用

在光源剔除阶段（Compute Shader），我们需要存储：

-   每个 **tile** 的光源数量
    
-   每个 **tile** 的光源索引列表

这些数据量可能非常大，并且是 GPU 动态生成的 → 这时就要用 **SSBO** 来存。

### 举例

GLSL 里你会看到类似代码：
```glsl
// 定义一个 SSBO，存每个 tile 的光源索引
layout(std430, binding = 0) buffer LightIndexBuffer {
    uint lightIndices[];
};

// 另一个 SSBO，存每个 tile 的光源数量和偏移
layout(std430, binding = 1) buffer TileMetaBuffer {
    uint tileOffsets[];
};

```
在 Compute Shader 里就能：

-   `lightIndices[...] = someLightID;`
    
-   `atomicAdd(tileOffsets[tileID], 1);`
    

这样 GPU 就直接把「tile → 光源表」构建好了，后面 fragment shader 就可以查这个 SSBO 来得到对应 tile 的光源。
...
-   可以理解为「GPU 端的大号数组」，Shader 可以读写。
    
-   特别适合用在 **Tiled/Clustered Shading** 里存放光源表、粒子系统数据、GPU 计算结果等。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0ODYxNDgxOTAsMzY1MTc5NjcsODI0Nz
IyMDI3XX0=
-->
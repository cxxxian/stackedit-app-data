...
一维dp做法
和01背包一样依旧不用初始化
和01背包对比，第二层for循环无需从尾部往前了，因为可以重复选择，
但是从前往后的话就要加`if`判断是否能放
```cpp
#include <iostream>
#include <vector>
using namespace std;
...
```
# 零钱兑换
因为求的是最少硬币数量，所以求排列和求组合的做法都可以，因为硬币数量是一样的
...
因为可以反复卖出，所以持有的时候也可能是上一次没持有，当前买入了
```cpp
class Solution {
public:
    int maxProfit(vector<int>& prices) {
...
```
# 买卖股票的最佳时机III
五维，分别对应：不操作，第一次买入，第一次卖出，第二次买入，第二次卖出
```cpp
class Solution {
public:
    int maxProfit(vector<int>& prices) {
...
![输入图片说明](/imgs/2026-02-08/ZwD2C6gsAAUAUlpk.png)
...
![输入图片说明](/imgs/2026-02-08/jhELhwyrgVgpW4FZ.png)
...
![输入图片说明](/imgs/2026-02-08/N9lb1hapGnxouSxY.png)
...
# 回文子串
双指针的办法，比较简单，就是要注意分两种情况的双指针，而且是向外扩散的双指针，而不是从外往内的判断
```cpp
class Solution {
public:
...
# 最长的回文子序列
如果相等就是`dp[i + 1][j - 1]`的长度+2，如果不相等的话就是单边找最大的：`max(dp[i + 1][j], dp[i][j - 1])`
为什么`j`要从`i + 1`开始遍历，因为`i`是左边界，`j`是右边界，`i==j`的情况已经初始化为`1`了。
```cpp
class Solution {
public:
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU1Mjc3OTg5MSwtMTY0NTcyNzA0OCwtMj
A2Nzc5MjM3OSwtNjAzMjIxMzg4LDcwODk3MjM2MCwxMTc5NTE2
Nzc0LDQ4NzQwMzkxMywtNzcwODg4Mzg5LDE4MDQxNTU4NzksLT
kxNjIxNTA5NSwtMTEzMjA5NjIyNCwxMTEyNTkyNjk1LC0xMDk0
MzE1OTk2LC0yMTI5NjUzMzA4LDE1MTc2NTQ0NDIsMzE5NzI4Mj
MyLDEwNDM1NTI5MzgsLTI5MjQ5OTU1MCwtOTgzNjA5MjcxLC04
MTYyMjA3NTNdfQ==
-->
...
# 海岸线计算
就是正常的`bfs`，然后判断`nextx`和`nexty`是陆地还是水还是边界，超出边界那就说明是海岸线，或者是水的情况也是海岸线（因为当前格子肯定是陆地），`count++`，然后遇到陆地就再加入`que`中循环判断
```cpp
#include <iostream>
#include <vector>
#include <queue>
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg3ODUyMzYyMSwtMTEyMDA4ODI1OCwtMj
AyOTA4MjA4Nyw3MzkwMTk3OTgsLTQ3MjI1MjE1MywxNzg0NDIw
OTA3LC04MzAyNDg0NjNdfQ==
-->
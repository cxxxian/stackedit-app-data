...
```cpp
sWidth = w; sHeight = h; sPixels.assign(data, data + w * h * 4);
```
...
```cpp
glTexImage2D(..., data); stbi_image_free(data);
```
...
```cpp
for (int i = 0; i < w * h * 4; ++i)     sPixels[i] = data[i];
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIyNDUxNjIwMSwxNjUwODA4MjYwLDExOD
A4MzE2OTZdfQ==
-->
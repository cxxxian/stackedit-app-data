...
struct Foo {
    ...
    ~Foo() { cout << "~Foo()\n"; }
    void show() { cout << "I am Foo\n"; }
};

int main() {
    unique_ptr<Foo> p1 = make_unique<Foo>(10); // 独占
    p1->show();

    // unique_ptr 不能复制，只能转移所有权
    unique_ptr<Foo> p2 = move(p1);
    if (!p1) cout << "p1 is empty now\n";
    p2->show();
}
...
int main() {
...

    {
        ...
        if (auto spt = wp.lock()) { // lock 返回 shared_ptr
...
template<typename T> void wrapper(T&& arg) {     consume(std::forward<T>(arg)); // 保留原始值类别 
}
```
这样：
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyNjMwMDE0MzksMTczOTk2MDcwOSwtMT
IzNTcwMzg3MCwtMTE0MzkyMjEwNV19
-->
...
`void wrapper(std::unique_ptr<int> p) {     process(p);  // ❌ 编译错误 }`
...
`std::unique_ptr<int> a(new int(10)); std::unique_ptr<int> b = a;        // ❌ 错：不能拷贝！ std::unique_ptr<int> b = std::move(a);  // ✅ OK：可以移动`
...
`process(p);              // ❌ 错误，尝试拷贝 process(std::move(p));   // ✅ 正确，移动传递`
...
> 它只能“移动构造”，而不能“拷贝构造”。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY4NTAxODczXX0=
-->
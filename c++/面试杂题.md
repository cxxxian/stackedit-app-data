...
```
3.  **泛型编程中处理特殊逻辑**
-   STL 内部经常用偏特化，比如处理 `std::allocator<void>`、`std::iterator_traits` 等

## 总结
-   **模板特化** = 针对特定类型或类型模式定制模板实现
-   **全特化** = 完全指定某个类型
-   **偏特化** = 指定类型模式（如指针、引用、数组等）
-   作用：
    -   提供特殊实现
    -   优化性能
    -   实现类型 traits

> “模板特化允许对特定类型或类型模式提供定制化实现，保持模板通用性的同时处理特殊情况。全特化针对某个类型，偏特化针对一类类型模式。常用在类型 traits、性能优化、STL 内部泛型编程中。”
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg2MjE0NDQ1OCwtNzM4MzQwMTM2LDE5Mz
EzNTk3MzJdfQ==
-->